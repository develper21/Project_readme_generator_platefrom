          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
              {roadmapPhases.slice(0, 4).map((phase, index) => (
                <div key={index} className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-base text-[#111827]">{phase.title}</h4>
                    <span className="block text-xs md:text-sm text-gray-500 mt-1">{phase.desc}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col items-center mt-12">
              <div className="flex items-start gap-3">
                <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#3B82F6] mt-1"></span>
                <div>
                  <h4 className="font-bold text-base text-[#111827]">{roadmapPhases[4].title}</h4>
                  <span className="block text-xs md:text-sm text-gray-500 mt-1">{roadmapPhases[4].desc}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="mt-12 w-full max-w-[900px] mx-auto">
          ...
          <div className="space-y-3">
            {faqList.map((faq, index) => (
              <details key={index} className="bg-white rounded-full px-4 py-3 shadow-sm group cursor-pointer">
                <summary className="flex items-center gap-3 list-none w-full cursor-pointer">
                  ...
                  <span className="font-medium text-sm text-[#222] flex-1">{faq.question}</span>
                  <FiArrowRight className="w-5 h-5 text-[#2563eb] group-open:rotate-90 transition-transform" />
                </summary>
                <div className="pl-10 pr-2 pt-2 text-sm text-gray-600">{faq.answer}</div>
              </details>
            ))}
          </div>
        </section>
